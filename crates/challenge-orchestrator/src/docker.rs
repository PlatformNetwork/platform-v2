...
impl DockerClient {
    ...
    ///
    /// Parameters:
    /// - `prefix`: Container name prefix to match (e.g., "challenge-")
...
mod tests {
    ...
    async fn test_cleanup_stale_containers_filters_entries() {
        ...
        bridge.set_containers(vec![
            make_container_summary("old", "challenge-old", now - 10_000),
            make_container_summary("exclude", "platform-helper", now - 10_000),
            make_container_summary("young", "challenge-young", now - 100),
        ]);
        let client = DockerClient::with_bridge(bridge.clone(), "platform-network");

        let result = client
            .cleanup_stale_containers("challenge-", 120, &["platform-"])
            .await
            .unwrap();
