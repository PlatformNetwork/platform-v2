...
impl ConsensusEngine {
    ...
    /// Select validators for review assignment
    ///
    /// Uses a deterministic seed derived from the submission ID and epoch
    /// to select validators for LLM and AST review. Returns two lists:
    /// 3 validators for LLM review and 3 for AST review.
    pub fn select_review_validators(
        &self,
        submission_id: &str,
        epoch: u64,
    ) -> ([Hotkey; 3], [Hotkey; 3], [u8; 32]) {
        let mut hasher = Sha256::new();
        hasher.update(submission_id.as_bytes());
        hasher.update(epoch.to_le_bytes());
        let seed: [u8; 32] = hasher.finalize().into();

        let validators: Vec<Hotkey> = self
            .validator_set
            .active_validators()
            .into_iter()
            .map(|r| r.hotkey)
        ...
        let select_n = |offset: usize| -> [Hotkey; 3] {
            if validators.is_empty() {
                return [Hotkey([0u8; 32]), Hotkey([0u8; 32]), Hotkey([0u8; 32])];
            }
            ...
            while selected.len() < 3 && selected.len() < validators.len() {
                state = state
                    .wrapping_mul(6364136223846793005)
                    .wrapping_add(1442695040888963407);
                let idx = (state >> 33) as usize % validators.len();
                let candidate = &validators[idx];
        ...
        let ast_reviewers = select_n(8);

        (llm_reviewers, ast_reviewers, seed)
    }
}
